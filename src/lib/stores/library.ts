...
const trackMetadataCache = new LRUCache<number, TrackMetadata>(
  CACHE_CONFIG.MAX_METADATA_CACHE,
);
const albumMetadataCache = new LRUCache<number, AlbumMetadata>(
  CACHE_CONFIG.MAX_METADATA_CACHE,
);

// Album art cache (separate, heavy data)
const albumArtCache = new LRUCache<number, string>(
  CACHE_CONFIG.MAX_ALBUM_ART_CACHE,
  {
    onEvict: (albumId, url) => {
      revokeBlobUrl(url);
    },
...
const trackCoverCache = new LRUCache<number, string>(
  CACHE_CONFIG.MAX_TRACK_COVER_CACHE,
  {
    onEvict: (trackId, url) => {
      revokeBlobUrl(url);
    },
...
export async function getFullTrack(
  ...
  if (
    preferBase64 &&
    track.track_cover &&
    track.track_cover.startsWith("blob:")
  ) {
    try {
...
export function getTrackAlbumCover(trackId: number): string | null {
  ...
  const customTrack = getCustomArtworkSync(
    get(customArtworks),
    "track",
    trackId,
  );
  ...
  if (metadata.album_id) {
    const customAlbum = getCustomArtworkSync(
      get(customArtworks),
      "album",
      metadata.album_id,
    );
...
export async function loadMoreTracks(): Promise<boolean> {
  ...
  isLoading.set(true);
  try {
    const newTracks = await getTracksPaginated(
      CACHE_CONFIG.TRACK_BATCH_SIZE,
      offset,
    );
...
export async function loadMoreAlbums(): Promise<boolean> {
  ...
  isLoading.set(true);
  try {
    const newAlbums = await getAlbumsPaginated(
      CACHE_CONFIG.TRACK_BATCH_SIZE,
      offset,
    );
