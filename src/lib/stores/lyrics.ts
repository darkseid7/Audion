...
async function saveSourceLyrics(
  ...
  try {
    await invoke("save_source_lyrics_file", {
      musicPath,
      sourceId,
      format,
      content,
...
async function refreshAvailableSources(
  musicPath: string,
  isStream = false,
): Promise<string[]> {
  try {
    const [cached, embeddedResult] = await Promise.all([
      invoke<Array<{ source_id: string; format: string }>>(
        "get_cached_sources",
        { musicPath, sourceIds: SOURCE_IDS },
      ),
      isStream
...
export async function fetchLyricsForTrack(): Promise<void> {
  ...
  try {
    ...
    if (userFile && fetchId === currentFetchId) {
      const result = await reparseFromCache(
        userFile.content,
        userFile.format,
        "user",
      );
    ...
    if (track.path && !track.source_type) {
      try {
        const embedded = await invoke<{
          content: string;
          synced: boolean;
        } | null>("get_embedded_lyrics", { musicPath: track.path });
...
export async function switchLyricsSource(sourceId: string): Promise<void> {
  ...
  const label =
    LYRICS_SOURCES.find((s: LyricsSource) => s.id === sourceId)?.label ??
    sourceId;
...
export async function importLyricsContent(
  content: string,
  format: "lrc" | "ttml" | "srt" | "json",
): Promise<void> {
  const track = get(currentTrack);
  if (!track) {
  ...
  try {
    if (format === "json") {
      const testLines = await invoke<unknown[]>("parse_apple_lyrics_json_cmd", {
        raw: content,
      });
      ...
      if (testLines.length === 0) {
        addToast(
          `No lyric lines found in the ${format.toUpperCase()} file.`,
          "error",
        );
        return;
      }
...
export const lyricsStore = {
  ...
  async clearSourceCache(sourceId: string): Promise<void> {
    ...
    if (!track) return;
    try {
      await invoke("delete_source_lyrics_file", {
        musicPath: track.path,
        sourceId,
      });
    } catch {
...
export async function getCurrentTrackActiveLyric(): Promise<{
  line: LyricLine;
  index: number;
} | null> {
  const track = get(currentTrack);
